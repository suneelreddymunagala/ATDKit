...
NSString *const kFAOBD2PIDRPM                      = @"0C";
NSString *const kFAOBD2PIDPedalPosition            = @"11";
NSString *const kFAOBD2PIDTorqDemand               = @"00";
NSString *const kFAOBD2PIDEngineTorq               = @"62";
...
//        self.sensorPIDsToScan = @[kFAOBD2PIDMassAirFlow, kFAOBD2PIDVehicleSpeed, kFAOBD2PIDVehicleFuelLevel, kFAOBD2PIDEngineCoolantTemperature, kFAOBD2PIDAmbientAirTemperature, kFAOBD2PIDAirIntakeTemperature, kFAOBD2PIDControlModuleVoltage,kFAOBD2PIDRPM,kFAOBD2PIDPedalPosition,kFAOBD2PIDTorqDemand,kFAOBD2PIDEngineTorq];
        self.sensorPIDsToScan = @[kFAOBD2PIDVehicleSpeed,kFAOBD2PIDPedalPosition,kFAOBD2PIDRPM];
...
-(void)restartStreaming{
    ...
    [self performSelector:@selector(sendInitialATCommands1) withObject:nil afterDelay:0.3];
    [self performSelector:@selector(sendInitialATCommands2) withObject:nil afterDelay:0.6];
    [self performSelector:@selector(streamPIDs) withObject:nil afterDelay:0.9];
}
...
- (void)sendInitialATCommands2
{
    ...
    NSString *message  = [NSString stringWithFormat:@"ATSP0\r"];
    NSData *data = [[NSData alloc] initWithData:[message dataUsingEncoding:NSASCIIStringEncoding]];
    [self.outputStream write:[data bytes] maxLength:[data length]];
}
...
- (void)stream:(NSStream *)theStream handleEvent:(NSStreamEvent)streamEvent {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"StreamEventchanged" object:[NSNumber numberWithInteger:streamEvent]];
    switch (streamEvent) {
...
- (void)parseResponse:(NSString *)response
{
    ...
    if ([responseSensorID isEqualToString:kFAOBD2PIDRPM]) {
//        NSLog(@"Akshay here");
    }
...
//            DLog( @"GPH: %0.1f", gph );
            ...
            CGFloat kmph = [byteValues[0] intValue];
...
//            DLog( @"Fuel Level: %0.1f", fl );
        ...
        else if ([responseSensorID isEqualToString:kFAOBD2PIDRPM] ) {
            CGFloat fl = [byteValues[0] intValue];
            
//            DLog( @"RPM: %0.1f", fl );
            
            [[NSNotificationCenter defaultCenter] postNotificationName:kFAOBD2PIDDataUpdatedNotification object:@{@"sensor":kFAOBD2PIDRPM, @"value":[NSNumber numberWithDouble:fl]}];
        }
        else if ([responseSensorID isEqualToString:kFAOBD2PIDPedalPosition] ) {
            CGFloat fl = [byteValues[0] intValue];
            
//            DLog( @"RPM: %0.1f", fl );
            
            [[NSNotificationCenter defaultCenter] postNotificationName:kFAOBD2PIDDataUpdatedNotification object:@{@"sensor":kFAOBD2PIDPedalPosition, @"value":[NSNumber numberWithDouble:fl]}];
        }
        else if ([responseSensorID isEqualToString:kFAOBD2PIDEngineTorq] ) {
            CGFloat fl = [byteValues[0] intValue];
            
            //            DLog( @"RPM: %0.1f", fl );
            
            [[NSNotificationCenter defaultCenter] postNotificationName:kFAOBD2PIDDataUpdatedNotification object:@{@"sensor":kFAOBD2PIDEngineTorq, @"value":[NSNumber numberWithDouble:fl]}];
        }
        else if ([responseSensorID isEqualToString:kFAOBD2PIDTorqDemand] ) {
            CGFloat fl = [byteValues[0] intValue];
            
            //            DLog( @"RPM: %0.1f", fl );
            
            [[NSNotificationCenter defaultCenter] postNotificationName:kFAOBD2PIDDataUpdatedNotification object:@{@"sensor":kFAOBD2PIDTorqDemand, @"value":[NSNumber numberWithDouble:fl]}];
        }
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:kFAOBD2PIDDataUpdatedNotification object:@{@"sensor":@"NO DATA", @"value":[NSNumber numberWithDouble:0]}];
